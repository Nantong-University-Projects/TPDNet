...
from .lane_directional_attention import LaneDirectionalAttention
...
class ROIGather(nn.Module):
    ...
    def __init__(self,
        ...
        self.use_directional_attention = use_directional_attention
        ...
        if self.use_directional_attention:
            self.lane_attention = LaneDirectionalAttention(
    ...
    def roi_fea(self, x, layer_index):
        ...
        # Apply lane directional attention if enabled
        if self.use_directional_attention:
            cat_feat = self.lane_attention(cat_feat)
