...
class Runner(object):
    ...
    def resume(self):
        # 恢复训练：加载模型、优化器、调度器和记录器的完整状态
        if self.cfg.resume_from:
            if os.path.exists(self.cfg.resume_from):
                self.recorder.logger.info('Resume training from: {}'.format(self.cfg.resume_from))
                ...
                self.recorder.logger.info('Resume from epoch: {}'.format(start_epoch))
                self.recorder.epoch = start_epoch - 1  # 因为train()中会+1
            else:
                self.recorder.logger.warning('Resume checkpoint not found: {}'.format(self.cfg.resume_from))
        # 加载预训练模型（仅模型权重）
        elif self.cfg.load_from or self.cfg.finetune_from:
