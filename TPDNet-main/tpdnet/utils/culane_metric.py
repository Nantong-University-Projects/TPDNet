...
def eval_predictions(pred_dir,
    ...
    if sequential:
        ...
        try:
            # 限制进程数，避免过多进程导致问题
            num_workers = min(cpu_count(), 16)  # 最多16个进程
            ...
            with Pool(num_workers) as p:
                try:
                    ...
                    # 如果发生管道错误或中断，尝试优雅关闭
                    logger.warning(f'Multiprocessing interrupted: {e}, trying to close pool gracefully...')
                    p.terminate()
                    p.join(timeout=5)
                    # 回退到顺序处理
            ...
            # 如果多进程完全失败，回退到顺序处理
            logger.warning(f'Multiprocessing failed: {e}, using sequential processing...')
            results = list(map(
                partial(culane_metric,
                        width=width,
                        official=official,
